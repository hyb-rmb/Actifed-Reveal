...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		// Target jumlah gosokan untuk dianggap terbuka
		let brushSize = 30;       // ukuran lingkaran gosok (pixel)
		let requiredBrushCount = 200; // jumlah gosokan minimal
		let currentBrushCount = 0;
		
		// Set mask pada overlay
		this.overlay.mask = maskShape;
		
		// Event mouse/touch down
		stageRef.on("stagemousedown", function(evt) {
		    isDrawing = true;
		    drawDot(evt.stageX, evt.stageY);
		});
		
		// Event mouse/touch move
		...
		// Event mouse/touch up
		stageRef.on("stagemouseup", function(evt) {
		    isDrawing = false;
		});
		
		// Fungsi menggambar lingkaran gosokan
		function drawDot(x, y) {
		    maskShape.graphics.beginFill("#000").drawCircle(x, y, brushSize);
		    currentBrushCount++;
		
		    // Cek apakah sudah cukup digosok
		    if (!revealed && currentBrushCount >= requiredBrushCount) {
		        revealed = true;
		        stageRef.gotoAndPlay("win"); // pindah ke frame label 'win'
		    }
		}
	}
