...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		stageRef.on("stagemousedown", function(evt) {
		    ...
		    stageRef.update(); // Update stage untuk immediate feedback
		});
		...
		// Buat canvas offscreen untuk perhitungan
		let checkCanvas = document.createElement("canvas");
		checkCanvas.width = overlay.image.width;
		checkCanvas.height = overlay.image.height;
		let checkCtx = checkCanvas.getContext("2d");
		
		checkInterval = setInterval(function() {
		    if (revealed) return;
		    
		    // Gambar mask ke canvas check
		    checkCtx.clearRect(0, 0, checkCanvas.width, checkCanvas.height);
		    checkCtx.save();
		    checkCtx.translate(-overlay.x, -overlay.y);
		    maskShape.draw(checkCtx);
		    checkCtx.restore();
		    
		    // Ambil pixel data
		    let imgData = checkCtx.getImageData(0, 0, checkCanvas.width, checkCanvas.height).data;
		    
		    let opaquePixels = 0;
		    let totalPixels = imgData.length / 4;
		    
		    // Hitung pixel yang tergores (hitam di mask = terbuka)
		    for (let i = 3; i < imgData.length; i += 4) {
		        // Kita cek jika pixel tidak transparan (di mask)
		        if (imgData[i] > 0) {
		            opaquePixels++;
		        }
		    }
		    
		    let percentRevealed = (opaquePixels / totalPixels) * 100;
		    
		    console.log("Revealed: " + percentRevealed.toFixed(2) + "%");
		    
		    if (percentRevealed >= targetRevealPercent) {
		        revealed = true;
